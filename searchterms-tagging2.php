...
function pk_stt2_admin_activation()
{
    ...
    $plugin_settings = ['max' => PK_MAX_SEARCH_TERMS, 'auto_add' => PK_AUTO_ADD, 'auto_link' => PK_AUTO_LINK,
	...
	<div class="postbox-container" style="width: 74%;">
						...
						<form id="stt2-options" method="post" action="">
							<table class="stt2-table">
								<tr>
									...
									<td>
										<input type="checkbox" <?php if (1 == get_option('pk_stt2_enabled')) {
        echo 'checked';
        ...
        echo 'checked';
								...
								<tr>
									...
									<td>
										<Input type="radio" name="auto_tag" value="1" <?php if (1 == get_option('pk_stt2_auto_tag')) {
        echo 'checked';
    } ?> />         Yes&nbsp;&nbsp;
										<Input type="radio" name="auto_tag" value="0" <?php if (0 == get_option('pk_stt2_auto_tag')) {
        echo 'checked';
    } ?> />         No
									</td>
								</tr>
...
function pk_stt2_function_prepare_searchterms($searchterms, $options, $popular=false)
{
    ...
    $toReturn .= (false == $popular) ? $options['list_header'] . $options['before_list'] : $options['before_list'];
    foreach ($searchterms as $term) {
        ...
        $toReturn .= (true == $options['show_count']) ? " ($term->meta_count)" . $options['after_keyword'] : $options['after_keyword'];
    }
    $toReturn = trim($toReturn, ', ');
...
function pk_stt2_onlist()
{
    ...
    if (1 == trim($_GET['onlist']) || 1 == $_GET['no']) {
        $pk_stt2_list = 2;
        update_option('onlist_status', $pk_stt2_list);
    }
    if ((('' != trim($_GET['activate']) && '' != trim($_GET['from'])) || '' != trim($_GET['activate_again'])) && 2 != $pk_stt2_list) {
...
function pk_stt2_registration_form($form_name, $submit_btn_txt='Register', $name, $email, $hide=0, $activate_again='')
{
    ...
    $wp_url           = (false === strpos($wp_url, 'http://')) ? get_bloginfo('siteurl') : $wp_url;
	...
	<form name="<?php echo $form_name; ?>" method="post" action="http://www.aweber.com/scripts/addlead.pl" <?php if (1 != $activate_again) {
        ?>onsubmit="return pk_stt2_validate_form_0()"<?php
    } ?>>
...
{
    $new_rules         = ['^search/(.+)\$' => 'index.php?s=' . $wp_rewrite->preg_index(1)];
